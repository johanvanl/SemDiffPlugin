...
public class JGitUtils {
	...
	private static void error(Throwable t, Repository repository,
			String pattern, Object... objects) throws Exception {
		...
		String err = MessageFormat.format(pattern, parameters.toArray())
				.toString();
		throw new Exception(err, t);
	}
		...
		try {
			...
			while (tw.next()) {
				...
				if (entmode != FileMode.GITLINK) {
					...
					ObjectLoader ldr = repository.open(ro.getId(),
							Constants.OBJ_BLOB);
			...
			if (throwError) {
				try {
					error(t, repository, "{0} can't find {1} in tree {2}",
							path, tree.name());
				} catch (Exception e) {
				}
			}
	 ...
	 * 
	 * This wasn't included by Gitblit.
	 * 
	 * @param repository
	 * @param commit
	...
	public static String getStringContent(Repository repository,
			RevCommit commit, String filePath, String... charsets)
			throws Exception {
		byte[] content = null;
		if (commit != null) {
			content = getByteContent(repository, commit.getTree(), filePath,
					true);
		} else {
			content = getByteContent(repository, null, filePath, true);
		}
		if (content == null) {
			return null;
		}
	...
	/**
	 * Get a list of all the files in the specified commit. This method is set
	 * to recursively walk through all the paths.
	 * 
	 * This method isn't a Gitblit method.
	 * 
	 * @param repository
	 * @param commit
	 * @return A List of all files in the specified commit.
	 * @throws Exception
	 */
	public static List<PathModel> getFilesInCommit(Repository repository,
	 ...
	 */
	public static List<PathChangeModel> getFilesChangedInCommit(
			Repository repository, RevCommit commit) {
		return getFilesChangedInCommit(repository, commit, true);
	}
	 ...
	 */
	public static List<PathChangeModel> getFilesChangedInCommit(
			Repository repository, RevCommit commit, boolean calculateDiffStat) {
			...
			if (commit.getParentCount() == 0) {
				...
				while (tw.next()) {
					list.add(new PathChangeModel(tw.getPathString(), tw
							.getPathString(), 0, tw.getRawMode(0), tw
	...
	public static List<PathModel> getDocuments(Repository repository,
		...
		try {
			...
			try {
				error(e, repository,
						"{0} failed to get documents for commit {1}",
						commit.getName());
			} catch (Exception e1) {
			}
		} finally {
	...
	private static PathModel getPathModel(TreeWalk tw, String basePath,
		...
		return new PathModel(name, tw.getPathString(), size, tw.getFileMode(0)
				.getBits(), objectId.getName(), commit.getName());
	}
	...
	public static List<RevCommit> getRevLog(Repository repository,
		...
		try {
			...
			try {
				error(t, repository,
						"{0} failed to get {1} revlog for path {2}", objectId,
						path);
			} catch (Exception e) {
			}
		}
	...
	public static List<RevCommit> searchRevlogs(Repository repository,
			...
			final com.gitblit.Constants.SearchType type, int offset,
			int maxCount) {
		List<RevCommit> list = new ArrayList<RevCommit>();
		...
		try {
			...
			try {
				error(t, repository,
						"{0} failed to {1} search revlogs for {2}",
						type.name(), value);
			} catch (Exception e) {
			}
		}
			...
			if (!target.startsWith(Constants.R_HEADS)) {
				...
				if (tagModels.size() > 0) {
					...
					for (RefModel tagModel : tagModels) {
						if (tagModel.getReferencedObjectId().getName()
								.equals(target)
	...
	public static boolean setBranchRef(Repository repository, String branch,
		...
		try {
			...
			try {
				error(t, repository, "{0} failed to set {1} to {2}",
						branchName, commitId);
			} catch (Exception e) {
			}
		}
	...
	public static boolean createTag(Repository repository, String objectId,
		...
		try {
			...
			try {
				error(e, repository,
						"Failed to create tag {1} in repository {0}", objectId,
						tag);
			} catch (Exception e1) {
			}
		}
