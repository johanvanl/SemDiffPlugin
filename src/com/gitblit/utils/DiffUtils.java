...
public class DiffUtils {
	 ...
	 * @return the diff
	 */
	public static DiffOutput getCommitDiff(Repository repository,
			RevCommit commit, DiffOutputType outputType) {
	 ...
	 * @return the diff
	 */
	public static DiffOutput getDiff(Repository repository, RevCommit commit,
	 ...
	 * @return the diff
	 */
	public static DiffOutput getDiff(Repository repository,
			RevCommit baseCommit, RevCommit commit, DiffOutputType outputType) {
	 ...
	 */
	public static DiffOutput getDiff(Repository repository,
			RevCommit baseCommit, RevCommit commit, String path,
	 ...
	 */
	public static String getCommitPatch(Repository repository,
			RevCommit baseCommit, RevCommit commit, String path) {
		String diff = null;
		try {
			final ByteArrayOutputStream os = new ByteArrayOutputStream();
	 ...
	 * @return patch as a string
	 */
	public static DiffStat getDiffStat(Repository repository,
			RevCommit baseCommit, RevCommit commit, String path) {
		DiffStat stat = null;
		try {
			RawTextComparator cmp = RawTextComparator.DEFAULT;
	 ...
	 */
	public static List<AnnotatedLine> blame(Repository repository,
			String blobPath, String objectId) {
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				AnnotatedLine line = new AnnotatedLine(commit, i + 1,
						rawText.getString(i));
				lines.add(line);
			}
		} catch (Throwable t) {
			try {
				error(t, repository, "failed to generate blame for {0} {1}!",
						blobPath, objectId);
			} catch (Exception e) {
			}
		}
	 ...
	 */
	public static NormalizedDiffStat normalizeDiffStat(final int segments,
			final int insertions, final int deletions) {
